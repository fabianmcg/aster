...
/// TableGen class for ISA-level instruction ops.
static constexpr StringRef AMDISAInstClassType = "AMDISAInstruction";

/// The TableGen class name for ASMArgFormat (from aster/IR/Inst.td).
static constexpr StringRef ASMArgFormatClassType = "ASMArgFormat";
...
private:
  using ArgTy = std::optional<std::pair<DagArg, StringRef>>;
  ...
  void emitArg(DagArg dagArg, StringRef printerCode,
...
ASMPrinterHandler::ASMPrinterHandler(const llvm::Record *rec)
  ...
  for (StringRef dagField : {"outputs", "inputs", "trailingArgs"}) {
    ...
    for (auto [i, arg] : llvm::enumerate(dag.getAsRange())) {
      if (!arg.getAsRecord()->isSubClassOf(ASMArgFormatClassType))
        continue;
      StringRef printerCode = arg.getAsRecord()->getValueAsString("asmPrinter");
      arguments[arg.getName()] = {arg, printerCode};
    }
  }
...
void ASMPrinterHandler::emitArg(DagArg dagArg, StringRef printerCode,
                                mlir::raw_indented_ostream &os) {
  ctx.withSelf("_inst.get" +
               llvm::convertToCamelFromSnakeCase(dagArg.getName(), true) +
               "()");
  os.printReindented(mlir::tblgen::tgfmt(printerCode, &ctx).str());
  os << "\n";
  ctx.withSelf("_inst");
}
...
void ASMPrinterHandler::genPrinter(raw_ostream &osOut) {
  ...
  // Build a flat list of (arch, encoding, syntax) entries.
  ...
  for (StringRef encId : encOrder) {
    // Collect all (arch, syntax) pairs for this encoding.
    llvm::SmallVector<std::pair<StringRef, StringRef>> archSyntaxPairs;
    for (const ArchEncodingSyntax &e : entries) {
      if (e.encodingId != encId)
        continue;
      archSyntaxPairs.push_back({e.archId, e.syntax});
    }

    os << "if (_enc == ::mlir::aster::amdgcn::Encoding::" << encId << ") {\n";
    os.indent();

    // Check if all arches share the same syntax.
    bool allSame = llvm::all_of(archSyntaxPairs, [&](const auto &pair) {
      return pair.second == archSyntaxPairs.front().second;
    });

    if (allSame) {
      emitSyntax(archSyntaxPairs.front().second, os);
    } else {
      for (const auto &[archId, syntax] : archSyntaxPairs) {
        os << "if (tgt.getTargetFamily() == ::mlir::aster::TypedEnum::get("
...
static bool generateInstAsmPrinters(const llvm::RecordKeeper &records,
  ...
  llvm::SmallVector<const llvm::Record *> instRecs =
      llvm::to_vector(records.getAllDerivedDefinitions(AMDISAInstClassType));
