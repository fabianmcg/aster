...
Three kernels exercise the main while-loop paths:
  ...
  - Zero trip: condition false on first evaluation; init value unchanged.

All tests use PASS_PIPELINE (default pipeline with scf_pipeline disabled).
The divergent test is parametrized over CDNA3 (gfx942) and CDNA4 (gfx950)
since it exercises the divergent save_cf_mask / restore_cf_mask path.
"""
